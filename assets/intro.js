...
(function () {
  ...
  // tower cranes on the tallest central plots (active-construction detail)
  var cranes = [];
  (function () {
    var central = buildings.filter(function (b) { return !b.civic; }).sort(function (a, b) { return b.hUnits - a.hUnits; });
    for (var ci = 0; ci < 3 && ci < central.length; ci++) {
      var b = central[ci];
      cranes.push({ gx: b.gx + b.w + 0.35, gy: b.gy + b.d * 0.5, bh: b.hUnits, order: b.order, seed: ci * 2.3 + 0.6, dir: ci % 2 ? 1 : -1 });
    }
  })();
  ...
  function drawCrane(cr, e, u) {
    var rs = 900 + (cr.order / orderMax) * 2400;
    var grow = easeOut(clamp01((e - (rs + 200)) / 1500));
    if (grow < 0.06) return;
    var topZ = (cr.bh * u * 1.15 + u * 2.7) * grow;
    var base = iso(cr.gx, cr.gy, 0);
    var mastTop = iso(cr.gx, cr.gy, topZ);
    var off = Math.max(2.5, u * fit * 0.13);
    // mast rails
    ctx.lineWidth = 1.2; ctx.strokeStyle = "rgba(121,230,242,0.6)";
    edge(base, mastTop);
    edge([base[0] + off, base[1]], [mastTop[0] + off, mastTop[1]]);
    // mast rungs (lattice)
    ctx.lineWidth = 0.8; ctx.strokeStyle = "rgba(121,230,242,0.32)";
    var segs = Math.max(5, Math.round(grow * 9));
    for (var s = 1; s < segs; s++) {
      var yb = [base[0] + (mastTop[0] - base[0]) * s / segs, base[1] + (mastTop[1] - base[1]) * s / segs];
      edge(yb, [yb[0] + off, yb[1]]);
    }
    if (grow < 0.52) return; // jib swings out once the mast is up
    var jp = clamp01((grow - 0.5) / 0.5);
    var theta = e * 0.00035 * cr.dir + cr.seed;      // slow slew
    var L = 3.0 * jp, Lc = 1.3 * jp;
    var mt2 = [mastTop[0] + off / 2, mastTop[1]];
    var jibTip = iso(cr.gx + Math.cos(theta) * L, cr.gy + Math.sin(theta) * L, topZ);
    var cjTip = iso(cr.gx - Math.cos(theta) * Lc, cr.gy - Math.sin(theta) * Lc, topZ);
    var apex = iso(cr.gx, cr.gy, topZ + u * 1.15);
    ctx.lineWidth = 1.2; ctx.strokeStyle = "rgba(121,230,242,0.6)";
    edge(mt2, jibTip); edge(mt2, cjTip);
    ctx.lineWidth = 0.8; ctx.strokeStyle = "rgba(121,230,242,0.4)";
    edge(apex, jibTip); edge(apex, cjTip); edge(mt2, apex);
    // counterweight
    ctx.fillStyle = "rgba(39,127,140,0.9)";
    ctx.fillRect(cjTip[0] - 4, cjTip[1] - 3, 8, 6);
    // trolley, hook cable and suspended load
    var tr = 0.4 + 0.35 * (0.5 + 0.5 * Math.sin(e * 0.0009 + cr.seed));
    var trolley = iso(cr.gx + Math.cos(theta) * L * tr, cr.gy + Math.sin(theta) * L * tr, topZ);
    var hookLen = (u * 1.4) * fit * (0.6 + 0.4 * (0.5 + 0.5 * Math.sin(e * 0.0011 + cr.seed)));
    ctx.lineWidth = 0.8; ctx.strokeStyle = "rgba(160,238,247,0.55)";
    edge(trolley, [trolley[0], trolley[1] + hookLen]);
    ctx.fillStyle = "rgba(232,162,92,0.85)";
    ctx.fillRect(trolley[0] - 3, trolley[1] + hookLen, 6, 4);
    // blinking aviation beacon
    var blink = Math.sin(e * 0.006 + cr.seed) > 0.2 ? 1 : 0.12;
    ctx.fillStyle = "rgba(240,90,70," + (0.9 * blink) + ")";
    ctx.beginPath(); ctx.arc(apex[0], apex[1], 2.4, 0, Math.PI * 2); ctx.fill();
  }
  ...
  function frame(now) {
   try {
    ...
    for (var ck = 0; ck < cranes.length; ck++) drawCrane(cranes[ck], e, u);
    ctx.restore();
